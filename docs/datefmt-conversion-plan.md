...
8. **Instant input (`dateTime`/`unixtime`) decomposes in the formatter's calendar**: a Flutter `DateTime` or a `unixtime` is a Gregorian *instant*. Mirroring JS `DateFactory._dateToIlib` → `DateFactory({unixtime, timezone, locale})`, `format()` builds the date **directly in the formatter's (locale's) calendar from the instant** (`_createCalendarDate(_calName, unixtime:, timezone:)`) — no Gregorian intermediate, no Julian-Day conversion. Do NOT decompose the instant into Gregorian `year/month/day` and label it with the locale's calendar; that shows raw Gregorian numbers with localized month names for non-Gregorian locales (am-ET → "23 የካቲት 2026" instead of "16 ሰኔ 2018"). Component inputs (`year/month/day`) keep the existing "already in the formatter's calendar" convention. Regression: `test/datefmt/datefmt_datetime_calendar_test.dart`.
